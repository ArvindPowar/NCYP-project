...
#import "HealthMedicalDetalisViewController.h"
#import "HealthmedicalInfoViewController.h"
@interface HealthMedicalViewController ()

@end

@implementation HealthMedicalViewController

@synthesize tableview,filedListArray,BgImg,appDelegate,backBtn,tabmenuView,refreshBtn,searchBtn,petrolBtn,changeLangBtn,tempLbl,usdpriceLbl,europriceLbl,poundpriceLbl;
    - (void)viewDidLoad {
        [super viewDidLoad];
        ...
        UIImageView * bannerImg=[[UIImageView alloc]initWithFrame:CGRectMake(screenRect.size.width*0.20,20,screenRect.size.width*0.70,screenRect.size.height*0.15)];
        [bannerImg setImage:[UIImage imageNamed:@"Banner_300x100_eng.png"]];
        ...
        //[refreshBtn addTarget:self action:@selector(createNewClient) forControlEvents:UIControlEventTouchUpInside];
        ...
        [petrolBtn setBackgroundImage:[UIImage imageNamed:@"news.png"]
                             forState:UIControlStateNormal];
        //[petrolBtn addTarget:self action:@selector(createNewClient) forControlEvents:UIControlEventTouchUpInside];
...
-(IBAction)backAction{
    ...
    {
        return [appDelegate.allfieldsimgArray count];
    }
    ...
    - (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
        ...
        testNameLbl.text=[appDelegate.allfieldsNameArray objectAtIndex:indexPath.row];
        ...
        subNameLbl.text=[appDelegate.allfielssubNameArray objectAtIndex:indexPath.row];
        ...
        if ([prefs stringForKey:@"BackgroundColor"]!=nil) {
            ...
            NSString *imgStr=[appDelegate.allfieldsimgArray objectAtIndex:indexPath.row];

        UIImageView * bannerImg;
        if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad){
            bannerImg=[[UIImageView alloc]initWithFrame:CGRectMake(15,15,screenRect.size.width*0.10,screenRect.size.height*0.07)];
        }else{
            bannerImg=[[UIImageView alloc]initWithFrame:CGRectMake(15,15,screenRect.size.width*0.11,screenRect.size.height*0.07)];
        }
            [bannerImg setImage:[UIImage imageNamed:imgStr]];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.row==0) {
        HealthMedicalDetalisViewController *health=[[HealthMedicalDetalisViewController alloc] initWithNibName:@"HealthMedicalDetalisViewController" bundle:nil];
        appDelegate.healthmedidetalisimgArray=[[NSMutableArray alloc]initWithObjects:@"pharmacies_on_call.png",@"pharmacies.png", nil];
        appDelegate.healthmedidetalisTxtArray=[[NSMutableArray alloc]initWithObjects:@"Pharmacies on call",@"Pharmacies",nil];
        [self.navigationController pushViewController:health animated:YES];

    }else if (indexPath.row==1){
        HealthMedicalDetalisViewController *health=[[HealthMedicalDetalisViewController alloc] initWithNibName:@"HealthMedicalDetalisViewController" bundle:nil];
        appDelegate.healthmedidetalisimgArray=[[NSMutableArray alloc]initWithObjects:@"private_hospitals.png",@"state_hospitals.png", nil];
        appDelegate.allfieldsNameArray=[[NSMutableArray alloc]initWithObjects:@"Private hospitals",@"State hospitals",@"Breakfast",@"Quick Bites",nil];
        [self.navigationController pushViewController:health animated:YES];

    }else if (indexPath.row==2){
        HealthmedicalInfoViewController *health=[[HealthmedicalInfoViewController alloc] initWithNibName:@"HealthmedicalInfoViewController" bundle:nil];
        appDelegate.healthmedInfoimgArray=[[NSMutableArray alloc]initWithObjects:@"kyrenia.png",@"nicosia.png",@"famagusta.png",@"morphou.png",@"trikomo.png", nil];
        appDelegate.healthmedicalTxtArray=[[NSMutableArray alloc]initWithObjects:@"Kyrenia",@"Nicosia",@"Famagusta",@"Morphou",@"Trikomo",nil];
        [self.navigationController pushViewController:health animated:YES];
