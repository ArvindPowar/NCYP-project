...
@synthesize tableview,filedListArray,BgImg,appDelegate,backBtn,tabmenuView,refreshBtn,searchBtn,petrolBtn,changeLangBtn,alert,tempLbl,usdpriceLbl,europriceLbl,poundpriceLbl,bannerImg,bannerAsyncimg,bannerImgBtn;
...
{
    return [appDelegate.plusSettingTXTArray count];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    testNameLbl.text=[appDelegate.plusSettingTXTArray objectAtIndex:indexPath.row];
    ...
    subNameLbl.text=[appDelegate.plusSettingTXTArray objectAtIndex:indexPath.row];
    ...
    NSString *imgStr=[appDelegate.plusSettingIMGArray objectAtIndex:indexPath.row];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    NSString *imgStr=[appDelegate.plusSettingTXTArray objectAtIndex:indexPath.row];
    if ([imgStr isEqualToString:@"Choose Theme"]) {
        if (indexPath.row==0) {
            alert = [[UIAlertView alloc]initWithTitle:@"NCYP" message:@"Please select one theme to apply your app" delegate:self cancelButtonTitle:@"Squre Theme" otherButtonTitles:@"Circle Theme", nil];
            [alert show];
            
        }

    }
    if ([imgStr isEqualToString:@"Background"]) {
        if (indexPath.row==1) {
            ChooseBackgroundViewController *background=[[ChooseBackgroundViewController alloc] initWithNibName:@"SettingViewController" bundle:nil];
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    if([title isEqualToString:@"Squre Theme"]){
        appDelegate.isSqurethemes=true;
        NSUserDefaults *prefs = [NSUserDefaults standardUserDefaults];
        [prefs setObject:@"Squre Theme" forKey:@"Theme"];
        [prefs synchronize];


    }
    if([title isEqualToString:@"Circle Theme"]){
        appDelegate.isSqurethemes=false;
        NSUserDefaults *prefs = [NSUserDefaults standardUserDefaults];
        [prefs setObject:@"Circle Theme" forKey:@"Theme"];
        [prefs synchronize];
